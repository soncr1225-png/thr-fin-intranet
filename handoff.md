...
> 최종 업데이트: 2026-04-26 (세션 14)

---

## 세션 14 작업 내역 (2026-04-26)

### 📊 대시보드 단계표시 인라인 포맷

**변경**: 단계표시가 두 줄로 표기되던 문제 → `5/7 입찰` 형식으로 한 줄 인라인 표시  
**수정 함수**: `FEE_stageCell` — 단계 숫자/이름을 column flex 2줄 레이아웃으로 정리, 막대와 gap 18px

### 📊 낙찰 결과 필터 수정

**변경**: 대시보드 "낙찰 결과·수수료 관리" 테이블에 입찰 진행 중(단계 5 이하) 사건도 표시되던 문제  
**수정**: `DASH_renderCaseList` 필터를 `FEE_inferStage(c) >= 6` 조건으로 변경 (대출 연결 이후만 표시)

### 👤 사건 테이블 고객명 표기

**변경**: 사건번호 앞에 고객명 표기 추가, 관리 버튼(CD_open)과 연동  
**수정**: `DASH_renderCaseList` 행 렌더에 `clientLabel` div 추가, `c.clientName` 연동

### 🔖 즐겨찾기 업데이트

- 굿옥션 → **옥션원** (`auction1.co.kr`)
- 호갱노노 → **아실** (`asil.kr`, 아파트 실거래가·시세 분석)
- 기타 유용 사이트에 **네이버 지도** (`map.naver.com`) · **네이버 부동산** (`land.naver.com`) 추가

### 🎨 대시보드 UI 가독성 개선

- 도식화(막대)와 단계표시 간격 확대 (gap 18px)
- 단계명 폰트 크기·굵기 조정 (15px, font-weight 900)
- **1차/2차 수수료 배지**: 클릭 가능한 pill 버튼으로 변경
  - 수령 완료: 초록 배경 `✓ 수령`
  - 미수령: 점선 테두리 회색 `미수령`
  - 클릭 시 `CD_open()` 호출

### 💰 수수료 수기 입력 기능 (`manualFee`)

**배경**: 낙찰가가 미입력인 경우 수수료가 `—`으로 표시되는 문제  
**해결**: CD 모달에 "수기 직접 입력" 체크박스 추가 → `manualFee` 필드로 저장  
- `manualFee > 0` 이면 `bidPrice × feeRate` 계산을 무시하고 이 금액을 수수료로 사용
- `FEE_SHADOW_FIELDS`에 `manualFee` 추가, `FEE_migrate`에서 복원
- 대시보드 테이블에서도 `c.manualFee` 우선 표시

**신규 함수**: `CD_toggleManualFee()`, `CD_onManualFeeInput()`

### 💬 MSG 수신함 ↔ 제보함 3방향 연동

**배경**: 메시지 수신함(MSG)과 제보함(BETA)이 완전히 별개 시스템으로 동작  
**해결**: 3방향 브릿지 구현
1. `BETA_sendReport` → MSG로 알림 발송 (제보함 제보 시 손청락에게 메시지 전송)
2. `MSG_send` → 오류신고/수정요청/기능추가 유형으로 손청락에게 보낼 때 제보함에도 자동 저장
3. 관리자 MSG 모달에 **제보함 탭** 추가 (`MSG_isAdmin` 조건) — 미처리 제보 카운트 뱃지 표시

**신규 함수**: `MSG_renderFeedback()`, `MSG_fbSetStatus()`

### 🗓 캘린더 네비게이션 순서

**변경**: `‹ › 2026년 4월` → `2026년 4월 ‹ ›` 순서로 날짜 레이블을 앞으로 이동  
**수정 위치**: 캘린더 헤더 HTML (cal-month-label div → 화살표 버튼 순서)

---

## 세션 13 작업 내역 (2026-04-25)

### 💬 메시지 네비 탭 추가 + 클릭 버그 수정

**증상**: 메시지 버튼이 어디 있는지 모르겠음 + 클릭해도 반응 없음

**원인**:
- 메시지 버튼이 헤더 우측 구석(로그인 후만 표시)과 더핀 회원 탭 내부에만 있어 찾기 어려움
- `MSG_open()`이 GAS fetch가 완료되기 전까지 모달 내용 미렌더링 → GAS 오류 상태면 빈 모달만 뜨거나 아무 반응 없는 것처럼 보임

**수정 (feature/msg-tab + feature/msg-fix)**:
- 상단 네비게이션 바에 `💬 메시지` 탭 추가 (자료실 옆)
  - 로그인 전 `display:none`, 로그인 후 `MSG_applyRole`에서 `display:flex`
  - 이모티콘(18px) **위**, "메시지" 텍스트 **아래** 세로 배치
  - 안읽은 메시지 빨간 뱃지 (`MSG_navBadge`) — fetch·markRead 시 동기화
- `MSG_open()` 수정: 즉시 `MSG_renderInbox()` 호출 후 백그라운드 GAS fetch
  - GAS 오류 상태에서도 모달이 즉시 열리고 캐시 데이터 표시
- 뱃지 3곳 동기화: 헤더(`MSG_unreadBadge`) · MBR 패널(`MBR_msgBadge`) · 네비 탭(`MSG_navBadge`)

---

## 세션 12 작업 내역 (2026-04-25)

### 💬 직원 메시지 v2 — 양방향 메시지 시스템 (fin-feature-mbr-msg)

**요청**: 직원들끼리 메시지를 주고받는 기능, 베타 피드백 수집 목적

**변경 내용 (index.html)**:
- MSG 시스템 전면 개편: 단방향(직원→관리자)에서 **양방향(전원↔전원)**으로
- 3탭 UI: 수신함 · 메시지 작성 · 보낸 메시지
- 발신 시 수신자 선택 드롭다운 (본인 제외 전 직원)
- 메시지 유형에 `업무전달` 추가, 베타 피드백 유형(오류신고·수정요청·기능제안)은 세부 화면에 베타 안내 멘트 표시
- 뱃지 이중 동기화: 헤더 메시지 버튼 + MBR 패널 `💬 메시지` 버튼 동기화
- MBR 통계카드 개선: 숫자 `36px`·세로 flex·hover 리프트
- MBR 헤더 서브타이틀 문구 제거
- `.msg-tabs` / `.msg-tab` CSS 추가 (탭바 스타일)

**변경 내용 (unified-gas.js)**:
- `sendMsg`: `to` 필드(받는사람) 추가, 시트 스키마 7열로 확장
- `getMsgs`: `user`·`admin` 쿼리 파라미터로 수신함/발신함 분리 반환
- 구형 6열 시트 자동 호환 (to 없으면 '전체' 처리)
- `markMsgRead`: `getLastColumn()`으로 읽음 열 동적 감지

**주의**: GAS 배포 후 구글 시트의 '메시지' 탭에 `받는사람` 헤더 열이 추가됨.  
기존 데이터가 있으면 구형 6열 포맷 그대로 유지해도 동작함.
