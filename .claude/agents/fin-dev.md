...
## 개발 규칙
1. 수정 전 반드시 `git status` 확인 — 미커밋 변경이 있으면 먼저 처리
2. 모든 기능은 **워크트리 기반 개발** → 완성 후 main 머지 (CLAUDE.md 워크플로우 참조)
3. `let`/`const` 선언은 사용 전에 배치 — TDZ(Temporal Dead Zone) 방지
4. 수정 후 `node scripts/validate.js` 실행 — 검증 통과 후 커밋
