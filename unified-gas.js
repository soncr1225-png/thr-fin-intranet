...
function getBulk(ss) {
  try {
    ...
    var maRes      = getMeongdo(ss, SH_MYEONGDO_A);
    var mdRes      = getMeongdo(ss, SH_MYEONGDO_D);
...
function myeongdoGet(p) {
  try {
    var ss = casesSS();
    if (p.action === 'listActive') return json(getMeongdo(ss, SH_MYEONGDO_A));
    if (p.action === 'listDone')   return json(getMeongdo(ss, SH_MYEONGDO_D));
    return json({ ok: false, error: 'unknown myeongdo action' });
  } catch(e) { return json({ ok: false, error: e.message }); }
}
...
function myeongdoPost(b) {
  try {
    ...
    var d  = b.data || {};
    switch (b.action) {
      case 'add':      return json(addMeongdo(ss, d));
      case 'update':   return json(updateMeongdo(ss, d));
      case 'complete': return json(completeMeongdo(ss, d));
      case 'delete':   return json(deleteMeongdo(ss, d));
      default:         return json({ ok: false, error: 'unknown myeongdo action' });
    }
  } catch(e) { return json({ ok: false, error: e.message }); }
}

function addMeongdo(ss, d) {
...
function addMeongdoNegotiationCol() {
  try {
    var ss = casesSS();
...
// 구버전 시트를 백업하고 신규 스키마로 재생성 — GAS 에디터에서 수동 실행
function migrateMeongdoSheet() {
  try {
    var ss = casesSS();
    [SH_MYEONGDO_A, SH_MYEONGDO_D].forEach(function(name) {
      var existing = ss.getSheetByName(name);
      if (existing) existing.setName(name + '_백업');
    });
    setupMeongdoSheet(ss.insertSheet(SH_MYEONGDO_A), false);
    setupMeongdoSheet(ss.insertSheet(SH_MYEONGDO_D), true);
    return { ok: true, message: '마이그레이션 완료. 기존 시트는 _백업으로 보존됨.' };
  } catch(e) { return { ok: false, error: e.message }; }
}

// 백업 시트에서 신규 스키마로 데이터 복원 — GAS 에디터에서 수동 실행
function restoreMeongdoFromBackup() {
  try {
    var ss          = casesSS();
    var activeSheet = sh(ss, SH_MYEONGDO_A);
    var doneSheet   = sh(ss, SH_MYEONGDO_D);

    var pairs = [
      { backupName: SH_MYEONGDO_A + '_백업', target: activeSheet, isDone: false },
      { backupName: SH_MYEONGDO_D + '_백업', target: doneSheet,   isDone: true  }
    ];
    ...
    pairs.forEach(function(pair) {
      var backupSheet = ss.getSheetByName(pair.backupName);
      if (!backupSheet) { notFound.push(pair.backupName); return; }

      var lastRow = backupSheet.getLastRow();
      if (lastRow < 2) return;

      var data    = backupSheet.getRange(1, 1, lastRow, backupSheet.getLastColumn()).getValues();
      var headers = data[0].map(function(h) { return String(h).trim(); });

      function ci(name) { return headers.indexOf(name); }
      var idxId           = ci('id');
      var idxCaseNo       = ci('caseNo');
      var idxPropertyName = ci('propertyName');
      var idxClientName   = ci('clientName');
      var idxNote         = ci('note');
      var idxCreatedAt    = ci('createdAt');
      var idxStaffList    = ci('staffList');

      for (var i = 1; i < data.length; i++) {
        var r = data[i];
        if (r.every(function(c) { return c === '' || c === null; })) continue;

        var id        = (idxId >= 0 && r[idxId]) ? String(r[idxId]) : Utilities.getUuid();
        var caseNo    = idxCaseNo       >= 0 ? String(r[idxCaseNo]       || '') : '';
        var address   = idxPropertyName >= 0 ? String(r[idxPropertyName] || '') : '';
        var staffName = idxStaffList    >= 0 ? String(r[idxStaffList]    || '').split(',')[0].trim() : '';
        var memo      = idxNote         >= 0 ? String(r[idxNote]         || '') : '';
        if (idxClientName >= 0 && r[idxClientName]) {
          memo = '[고객: ' + r[idxClientName] + '] ' + memo;
        }
        var createdAt = (idxCreatedAt >= 0 && r[idxCreatedAt]) ? r[idxCreatedAt] : new Date();

        pair.target.appendRow([
          id, caseNo, address,
          '',          // occupantType — 수동 확인 필요
          '내용증명',  // stage — 수동 조정 필요
          staffName,
          memo,
          pair.isDone ? 'done' : 'active',
          createdAt
        ]);
        totalRestored++;
      }
    ...
    return {
      ...
      message: '복원 완료: ' + totalRestored + '건 복원됨. 점유자유형·진행단계는 수동으로 확인하세요.',
      notFound: notFound
    };
  } catch(e) { return { ok: false, error: e.message }; }
}
...
function statsGet(p) {
  ...
  // 명도 데이터
  var mdA = getMeongdo(ss, SH_MYEONGDO_A).data || [];
  var mdD = getMeongdo(ss, SH_MYEONGDO_D).data || [];
